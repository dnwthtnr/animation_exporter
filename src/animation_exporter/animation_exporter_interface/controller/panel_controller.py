...
import sys
print(sys.path)
from . import ExportQueuesController, maya_process_delegator, item_detail_controller, SceneDataController
...
from utility_resources import keys, settings, cache

from src.PySideWrapper.src.PySideWrappers import base_layouts, base_widgets, styles, constants
...
class PanelController(QtCore.QObject):
    ...
    def add_to_active_export_queue(self, scene_data_dict):
        ...
        if False in [isinstance(_item, list) for _item in selected_animation_ranges]:
            ...
            _entry_scene_data_dict[
                keys.item_export_name_key] = export_name + f"_RANGE({selected_animation_ranges[0]}_{selected_animation_ranges[1]})"
        ...
        for _animation_range in selected_animation_ranges:
            ...
            _entry_scene_data_dict[
                keys.item_export_name_key] = export_name + f"_RANGE({_animation_range[0]}_{_animation_range[1]})"
