...
class __Scene_Controller(QtCore.QObject):
    ...
    def open_file(self, filepath):
        ...
        try:
            utils.executeDeferred(partial(cmds.file, filepath, open=True, force=True))
            logger.info(f'File successfully opened ')
            utils.executeDeferred(self.emit_scene_contents)
    ...
    def emit_item_details_on_thread(self, item):
        _item_data_dict = utils.executeDeferred(partial(self.emit_item_details_dictionary, item))
